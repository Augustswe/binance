...
- ⏳ **平仓冷静期实时倒计时**：行情表新增「冷静期」列，平仓后显示 ⏳ MM:SS 徽章每秒递减；冷静期内引擎拒绝开仓，拒绝原因与操作日志均带「冷静期 MM:SS」倒计时提示
...
risk:
  ...
  cooldown_minutes: 60              # 平仓冷静期(分钟): 平仓后 N 分钟内禁止同币种再次开仓, 行情表「冷静期」列显示 ⏳ MM:SS 实时倒计时
