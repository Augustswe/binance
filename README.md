...
| 区域 | 内容 |
|---|---|
| 顶部 | 心跳指示（主循环真实更新时间，超时变色预警）、模式徽章、运行状态、暂停/恢复、重置今日；**主网模式下额外显示红色闪烁横幅**提醒真实资金 |
...
- **心跳 💚**：右上角随时显示主循环最近一次真实 tick 时间；超过 10s 变黄、超过 1 分钟变红，用于判断服务是否还活着。
- **暂停 / 恢复、重置今日**：顶部按钮可暂停策略（不再开仓/平仓）、恢复运行；「重置今日」以当前权益为新起点并解除日亏熔断。
- **主网红色横幅**：切到主网后顶部出现红色闪烁横幅，时刻提醒正在动用真实资金。
- **策略信号 Tab**：按启用的策略模式分 Tab，点 Tab 切换查看该模式全部币种的信号（方向 / 信号强度 / 杠杆 / 距通道），带信号条数角标。
- **⚙️ 设置面板**：右上角打开，分区见下表；修改风控 / 网络 / 模式均热生效，无需重启。
- **模式卡片**：勾选启用 / 停用某策略，卡片实时显示该模式资金权重；同币种多模式「先到先得」。
- **开机自启开关**：设置面板底部，一键开启登录自启（仅 macOS / Windows）。
...
| 🍎 开机自启 | 开启后登录/开机自动启动服务（macOS 用 launchd，Windows 用「启动」文件夹；仅 macOS / Windows 支持） |
