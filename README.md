...
行情与持仓表的「主导策略」列**数据驱动**，显示每个币种当前真正主导的策略：

- **有持仓** → 显示该持仓的实际模式（`pos.mode`）；从交易所同步回来的持仓显示 `交易所同步`。
- **无持仓** → 在该币种所有 `模式:币种` 信号里，取**评分最高、有开仓动作优先**的模式（如 `Donchian` / `均线交叉` / `网格` / `RSI反转` / `布林带`）。不再千篇一律刷成同一值，也不再有 `--`。
...
- **已绑定 API 列表**（账户卡片，即多账户盈亏对比总览）：
  ...
  - **操作按钮顺序**：`打开面板` → `启用/停用` → `解绑` → `⚙️ 设置`（设置置末）。
    ...
    - **⚙️ 设置**：打开该账户的设置抽屉（见下）。
  - **横向柱状图**：按账户 `realized_pnl` 与按策略 `sum_realized` 对比（涨红跌绿）。
...
A: 数据驱动：有持仓显示该持仓实际模式（从交易所同步的持仓显示"交易所同步"）；无持仓显示该币种信号中评分最高/最优先触发的模式，不再是固定值或 `--`。
