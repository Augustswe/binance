...
- 🔒 **激进风控 + Web 实时调参**：单笔 ≤1000U、**总敞口按权益百分比控制（默认 40%，可选 20/40/60/80%；>40% 实时分级警告 + 引擎拦截）**、总持仓硬上限 2000U（仅兜底）、日亏损 30% 熔断、平仓冷静期；设置面板可**实时调整风控与敞口**并落盘 `config.yaml`，运行中热生效
...
risk:
  ...
  cooldown_minutes: 60              # 平仓冷静期(分钟): 平仓后 N 分钟内禁止同币种再次开仓; 冷静期倒计时仅显示在系统日志(拒绝原因 + 平仓事件), 不在行情表展示
  max_total_exposure_pct: 40        # ★主控制: 总敞口占权益上限 % (0=关闭, 改用固定值兜底); 开仓前按权益比例核对, >40% 设置面板分级警告
```
...
**敞口（总持仓名义价值）采用「百分比优先」模型**：开仓前与每轮对账后，总敞口上限 = `权益 × max_total_exposure_pct`；固定值 `max_total_position_notional` 仅在百分比关闭（`=0`）时才兜底生效。这样敞口随权益自动缩放——小账户不会被一个写死的 USDT 数字推到超高杠杆，大账户也不会被固定值静默压低你设的比例。

**可调控项**（直接决定持仓敞口）：

| 项 | 含义 | 默认 |
|---|---|---|
| **总敞口占权益 %** | **总持仓名义价值 ÷ 权益 的上限（主控制）**：开仓前按权益比例核对；>40% 实时分级警告（60% 琥珀激进 / 80% 红色极高风险；0=关闭改用固定值兜底） | **40%** |
| 总持仓硬上限（兜底） | `max_total_position_notional`：百分比开启时不约束敞口，仅作硬天花板 / 主网 t3 默认上限 | 2000 U |
| 单笔上限 | 单笔下单名义价值上限 | 1000 U |
| 单笔保证金 | 单笔保证金预算 × 杠杆 × 权重 = 实际名义仓位 | 200 U |
| 最大持仓数 | 同时持仓数上限 | 5 |
| 日亏熔断 | 当日回撤达到该百分比即熔断停机（面板填 %，如 30） | 30% |
| 冷静期 | 单币平仓后多久内不再开仓（倒计时仅显示在系统日志，不在行情表） | 60 分钟 |
| 杠杆 | auto 自动 / fixed 固定，及 min/max/fixed 倍率 | auto 1–5x |

> 参数与 `config.yaml` 的 `risk` / `leverage` 段一一对应；页面修改会**原地更新运行中的风控字典**，新开仓与 `check_open` 立即生效。敞口超限兜底 `enforce_exposure_cap`：`reduce` 自动减仓至上限内 / `halt` 熔断停机 / `off` 仅拦新开仓。
