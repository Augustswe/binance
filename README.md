...
- 🔒 **激进风控 + Web 实时调参**：三层防护（敞口百分比上限 / ATR 波动自适应止损 + 移动止损 / 日亏熔断 / 冷静期），详见 [🛡️ 风控与敞口](#风控与敞口)；设置面板可**实时调整风控与敞口**并落盘 `config.yaml`，运行中热生效
...
系统对「亏损 / 穿仓 / 控盘插针 / 极端行情」的防护分三层：**敞口与仓位限额 → 持仓中止损与市况过滤 → 熔断与冷静期**。所有参数在 `config.yaml` 的 `risk` / `leverage` / `position` 段，设置面板可**实时调整并热生效**，无需重启。
...
当前对**「控盘插针」「闪崩连环爆仓」**仍缺最关键的盘中防护，以下为待增强项（实现后补入上表）：

- **盘中波动率突变熔断**：单根 K 线/短时间内回撤 > X% 立即全局暂停开仓（日亏熔断按「当日权益」算，带杠杆遇闪崩可能未到阈值已穿仓）；
- **插针 / 影线过滤**：防止控盘长影线扫掉止损（以收盘价/实体确认或影线占比过滤）；
- **跳空穿仓保护**：单笔浮亏超硬上限立即市价减仓，避免跳空跳过止损以更差价成交；
- **资金费率 / OI / 多空比监控**：捕捉控盘与连环爆仓前兆（费率飙、OI 暴增）提前避让。
