...
- 🔒 **激进风控 + Web 实时调参**：四层防护（敞口百分比上限 / ATR 波动自适应止损 + 移动止损 / 日亏熔断 + 冷静期 / **盘中防护·四道防线**），详见 [🛡️ 风控与敞口](#风控与敞口)；设置面板可**实时调整风控与敞口**并落盘 `config.yaml`，运行中热生效
- ⏳ **平仓冷静期（倒计时仅在系统日志）**：平仓后 N 分钟内禁止同币种再次开仓；冷静期内引擎拒绝开仓，**拒绝原因与「平仓完成」事件日志均带「冷静期 MM:SS」实时倒计时**（不在行情表中展示）
- 🪟 **自定义确认弹窗（替代原生 confirm）**：立即平仓 / 清屏 / 重置账户 / 解绑账户 / 主网切换等不可撤销操作改用页面内自定义弹窗（危险操作为红色「确认」按钮），不再使用浏览器原生 `confirm()` 对话框
...
risk:
  ...
  max_total_position_notional: 2000 # 总持仓硬上限(兜底): 百分比开启时不约束敞口, 仅主网t3默认/百分比关闭时生效
  margin_per_position: 200          # 单笔保证金预算 (U) × 杠杆 = 实际名义仓位
  max_positions: 5                   # 同时持仓数上限
  daily_loss_stop: 0.30             # 日亏损 30% 熔断
  cooldown_minutes: 5                # 平仓冷静期(分钟): 平仓后 N 分钟内禁止同币种再开仓
  max_total_exposure_pct: 40         # ★主控制: 总敞口占权益上限 % (0=关闭, 改用固定值兜底); 开仓前按权益比例核对
  enforce_exposure_cap: "reduce"     # 超限兜底: reduce=自动减仓 / halt=熔断停机 / off=仅拦新开仓
```
...
系统对「亏损 / 穿仓 / 控盘插针 / 极端行情」的防护分四层：**敞口与仓位限额 → 持仓中止损与市况过滤 → 熔断与冷静期 → 盘中防护**。前 3 层见上；第 4 层 `盘中防护` 专门对抗「控盘插针」与「闪崩连环爆仓」，配置在 `config.yaml` 的 `guards:` 段，设置面板可**实时调整并热生效**，无需重启。

### 第 1 层：敞口与仓位限额（决定「最多亏多少」）

敞口（总持仓名义价值）= 各持仓 `数量 × 价格` 之和，衡量真实市场风险。采用**「百分比优先」模型**：总敞口上限 = `权益 × max_total_exposure_pct`；固定值 `max_total_position_notional` 仅在百分比关闭（`=0`）时才兜底。这样敞口随权益自动缩放——小账户不会被写死数字推到高杠杆，大账户也不会被固定值静默压低你设的比例。设置面板对敞口%有实时分级警告（越高越激进）。

| 参数 | 含义 | 默认 | 作用 |
|---|---|---|---|
| `max_total_exposure_pct` | **总敞口占权益上限（主控制）** | 40% | 开仓前 + 每轮对账后核对；超限由 `enforce_exposure_cap` 处理 |
| `max_total_position_notional` | 总持仓硬上限（兜底） | 2000 U | 百分比开启时不约束敞口；仅主网 t3 默认上限 / 百分比关闭时生效 |
| `max_single_order_notional` | 单笔名义价值上限 | 1000 U | 单笔下单单边上限 |
| `margin_per_position` | 单笔保证金预算 | 200 U | × 杠杆 × 权重 = 实际名义仓位 |
| `max_positions` | 同时持仓数上限 | 5 | 控制分散度（不限制跨标的相关系数） |
| `leverage.mode` | 杠杆策略 | auto | auto=按波动率自动 1x–5x / fixed=固定 `fixed` 倍 |
| `leverage.min/max` | 杠杆区间 | 1 / 5 | 强信号高倍重仓，弱信号低倍试探 |
| `enforce_exposure_cap` | 超限兜底动作 | `reduce` | `reduce`=自动减仓 / `halt`=熔断停机 / `off`=仅拦新开仓 |

> 单笔名义 = `保证金预算 × 杠杆 × 权重`；总敞口预算 = `min(单笔上限, 总持仓上限−已用, 保证金×杠杆×权重, 权益×敞口%−已用)`。对账后若继承/同步来超限持仓，`enforce_exposure_cap=reduce` 会自动把名义最大的持仓减回上限内（真正锁住敞口），`halt` 则直接熔断停机待人工。

### 第 2 层：持仓中止损 + 市况过滤（决定「单笔亏多少 / 何时不开」）

**ATR 波动率自适应止损**（核心）：止损距离 = `入场价 ∓ sl_atr × ATR`，波动越大距离自动变宽，不会被正常波动扫掉、也不会在猛涨猛跌时过小。`position.sl_atr ≈ 2.7`，Donchian 自身 `sl_atr = 2.5` 作回退。

**移动止损（trailing）**：盈利后止损线只上移（多）/下移（空），跟随极值 `∓ trail.atr_mult × ATR`（`atr_mult = 2.0`），锁住浮盈、让利润奔跑。

**止盈**：`入场价 ± tp_atr × ATR`（`tp_atr ≈ 3.9`），`min_tp_pct = 0.25%` 至少覆盖 2.5 倍手续费，防止止盈被手续费吃掉。

**市况（regime）过滤**：ML 选择器判定 `trend_up / trend_down / ranging`；**震荡市（`range`）抑制 Donchian 开仓**（趋势策略不在震荡里乱追）。ML 门禁（gate）当前**关闭**（回测 AUC≈0.52 无区分力），仅作预留。

**双保险止损**（断网也不丢单）：
1. **交易所侧**：止损单挂交易所（STOP_MARKET / Algo Order），本地崩溃、行情断流也能触发；
2. **本地侧**：每 `poll_interval`（5s）轮询，触发后市价平仓（`reduceOnly` 防反向开仓）。

### 第 3 层：熔断与冷静期（决定「何时彻底停下」）

| 参数 / 机制 | 含义 | 默认 | 触发 |
|---|---|---|---|
| `daily_loss_stop` | **日亏损熔断** | 30% | 当日权益回撤 ≤ −30% → 全局停机（`halt`），不再开仓、不平仓，待人工重置 |
| `enforce_exposure_cap: halt` | 敞口超限熔断 | — | 总敞口突破上限且设为 `halt` → 熔断停机 |
| `cooldown_minutes` | **平仓冷静期** | 5 分钟 | 平仓后 N 分钟内禁止同币种再开仓（防反复被扫）；拒绝原因带「冷静期 MM:SS」倒计时，仅记系统日志 |
| 外部爆仓检测 | 交易所持仓消失识别 | — | 本地有持仓、交易所无 → 识别为外部/强平平仓，本地同步结清（`record=False`，不计入冷静期） |

### 第 4 层：盘中防护（对抗「控盘插针」「闪崩连环爆仓」）

第 1–3 层是「按权益/按持仓」的防护，对**带杠杆遇闪崩、控盘长影线插针**仍有盲区。本层按「行情本身」急刹，由 `config.yaml` 的 `guards:` 段统一控制（全部默认开启，可单独关闭）：

| 防线 | 配置键 | 含义 | 默认 | 触发 / 动作 |
|---|---|---|---|---|
| **盘中波动率突变熔断** | `intrabar_halt` | 近 `lookback_seconds`(300s) 内某币 振幅(峰−谷)/峰 > `drawdown_pct`(6%) → **暂停所有开仓**；带迟滞，振幅回落到 `resume_pct`(3%) 以下才恢复，防阈值附近抖动。价格缓冲在内存、重启清零 | 开启 | 日亏熔断按「当日权益」算，带杠杆闪崩可能未到阈值已穿仓；此熔断按行情急刹兜底 |
| **插针 / 影线过滤** | `wick_filter` | ① 开仓信号过滤：触发行 `影线占比=(range−body)/range > max_wick_ratio`(0.7) 即**拦截开仓**（长影线/插针不追）；② 止损确认：`confirm_on_close`(true) 时，若当前 K 线只是插针扫过 SL 又收回（长影线），**不触发止损**，等收盘价真正跌破才认损 | 开启 | 防控盘长影线扫掉止损 |
| **跳空穿仓保护** | `gap_protect` | 单笔浮亏占 **保证金**(=名义/杠杆) 比例 ≥ `max_single_loss_pct`(40%) → **立即市价减仓**。基准用保证金（亏损%=价格跌幅×杠杆），与 ATR 软止损同源，故该硬上限**天然位于 ATR 止损之上**，只兜底「跳空跳过止损 / 行情断流」，不抢在 ATR 前平仓 | 开启 | ATR 软止损外的硬兜底，接住跳空以更差价成交 |
| **资金费率 / OI / 多空比监控** | `oi_monitor` | 每 `interval_seconds`(60s) 节流拉取各币 funding / OI / 多空比；`|资金费率| > funding_alert`(0.1%) 或 OI 较基准涨 > `oi_spike_pct`(30%) → 判异常。`action=suppress_open` 暂停开仓 / `action=warn` 仅预警 | 开启 | 捕捉控盘与连环爆仓前兆（费率飙、OI 暴增），提前避让 |

> **与既有防护的关系**：L4 是「盘中实时」维度补充，不替代 L1–L3。波动率熔断/OI 监控在 `RiskManager.check_open` 拦截新开仓（原因写入 `state` 的 `vol_halt` / `oi_alert_suppress` 标志，重启清零）；影线过滤贯穿开仓与止损确认；跳空保护在 `_check_tp_sl` 硬止损环（位于 TP/SL 之前）兜底。参数调整落盘 `config.yaml` 即热生效。
...
│   ├── guards.py          # 盘中防护: 波动率熔断/影线过滤/跳空穿仓/OI 监控 (四道防线)
